...
<head>
	...
	<link rel="stylesheet" href="<%=path %>/css/diy2_5/comment.css">
</head>
<body class="page">
...
<div class="popLayer hide">
	...
	<script src= "<%=path %>/js/lib/swiper-3.3.1.jquery.min.js"></script>
  	<script>
  		// 获取参数名
  		...
  		$(document).ready(function() {
			...
			var goodsType=getQueryString('goodsType');
  			...
  			var initData= new (function(){   // 进入页面加载评论数量
  				var init=function(){
  					...
  					$.ajax({
			        	url: 'evalNum2.do',
						data:{goodsId:goodsId,goodsType:goodsType},
			        	type: 'POST',
			        	dataType: 'json',
			    	    cache:false,
			    	    timeout:"60000",
  			...
  			var clickAjax=new (function(){ 	// 点击title
  				...
  				$('.comments-detail ul').css('min-height','92vh');
  					var _this = e.target;
  					if(e.target){
				    ...
				    $.ajax({
			        	...
			        	type: 'POST',
			        	dataType: 'json',
						data:{goodsId:goodsId,evalType:i,goodsType:goodsType},
			    	    cache:false,
			    	    timeout:"60000",
			    	    success:function(data){
			    	    	$box.empty();
			...
			clickAjax.queryData(0);
  			...
  			$(".commentPic").on("tap",function(e){
  				var _this = e.target;
  				var page = 1;
  				$('.comments-detail ul').css('min-height',0);		// 20161215
  				$com_btn.removeClass('commentClass-btn2');
  			    $(_this).closest(".commentClass-btn").addClass('commentClass-btn2');
  				$box.empty();
				$(".shaiTu").remove();
				...
				var url = 'evalPic2.do';
  				$.ajax({
		        	url: url,
		        	type: 'POST',
					data:{goodsId:goodsId,page:page,goodsType:goodsType},
		        	dataType: 'json',	
		    	    cache:false,
		    	    timeout:"60000",
		    	    success:function(data){
				...
				$shaiTu.parent().off('scroll');
  				$shaiTu.parent().on('scroll', function(e){
  					if($(this).scrollTop() + $(this).height() >= $(this).find('.shaiTu').height()){
  						if(loading) return;
  						loading = true;
  						$.ajax({
		        			url: url,
			 ...
			 function getCommentInfo(){
				...
				var showdata =function(){
				    $.ajax({
			    	    ...
			    	    cache:false,
			    	    timeout:"60000",
			    	    data:{page:page,goodsId:goodsId,evalType:index,goodsType:goodsType},
			    	    success:function(data){
			    	    	var str="";
		        			page++;
				        	if(data.code==200){
